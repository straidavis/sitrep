...
const Equipment = () => {
    ...
    const uniqueItems = useMemo(() => {
        console.log('[Equipment Filter] Selected Deployment IDs:', selectedDeploymentIds);
        console.log('[Equipment Filter] All equipment deployment IDs:', allEquipmentData.map(e => e.deploymentId));
        ...
        const filteredRaw = (selectedDeploymentIds && selectedDeploymentIds.length > 0)
            ? allEquipmentData.filter(item => {
                const match = selectedDeploymentIds.includes(item.deploymentId);
                if (!match && item.deploymentId) {
                    console.log('[Equipment Filter Debug]', {
                        selectedIds: selectedDeploymentIds,
                        itemDepId: item.deploymentId,
                        itemDepIdType: typeof item.deploymentId,
                        selectedIdTypes: selectedDeploymentIds.map(id => typeof id)
                    });
                }
                return match;
            })
                ...
                {(!selectedDeploymentIds || selectedDeploymentIds.length !== 1) ? (
                    <div className="space-y-4">
                        {Object.values(categoryItems.reduce((acc, item) => {
                            ...
                            const depId = depItems[0].deploymentId;
                            console.log('[Deployment Lookup] Looking for:', depId, 'Type:', typeof depId);
                            console.log('[Deployment Lookup] Available deployments:', deployments.map(d => `${d.id} (${d.name})`).join(', '));
                            const dep = deployments.find(d => String(d.id).trim() === String(depId).trim());
                            console.log('[Deployment Lookup] Found:', dep ? `${dep.id} - ${dep.name}` : 'NOT FOUND');
                            const depName = dep?.name || 'Unassigned / Unknown';
                            return (
