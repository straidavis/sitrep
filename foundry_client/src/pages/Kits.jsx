...
import { getAllKits, getAllKitItems, addKit, bulkAddKitItems, deleteKit, deleteKitItemsByKitId, updateKit, updateKitItem } from '../db/kits'; // Refactored
import { useAuth } from '../context/AuthContext';
import { useDeployment } from '../context/DeploymentContext';
import { Briefcase, Upload, FileSpreadsheet, ChevronDown, ChevronRight, Package, Trash2, AlertCircle, Save, Move } from 'lucide-react';
...
const Kits = () => {
    ...
    const [dragOverDeployment, setDragOverDeployment] = useState(null); // Track which deployment is being hovered
    ...
    const [kitToAssign, setKitToAssign] = useState(null);
    ...
    const loadKits = async () => {
        const loadedKits = await getAllKits();
    ...
    // --- Kit Drag & Drop (Reassignment) ---
    const handleKitDragStart = (e, kit) => {
        if (!canEdit) return;
        e.dataTransfer.setData('kitId', kit.id);
        e.dataTransfer.setData('originDeploymentId', kit.deploymentId);
        e.dataTransfer.effectAllowed = 'move';
    };

    const handleDeploymentDragOver = (e, deploymentId) => {
        if (!canEdit) return;
        e.preventDefault(); // Allow drop
        setDragOverDeployment(deploymentId);
    };

    const handleDeploymentDragLeave = (e) => {
        setDragOverDeployment(null);
    };

    const handleKitDrop = async (e, targetDeploymentId) => {
        e.preventDefault();
        setDragOverDeployment(null);
        const kitId = e.dataTransfer.getData('kitId');
        const originDeploymentId = e.dataTransfer.getData('originDeploymentId');

        if (!kitId || !canEdit) return;

        // If dropping on same deployment, do nothing
        if (String(originDeploymentId) === String(targetDeploymentId)) return;

        try {
            await updateKit(kitId, { deploymentId: targetDeploymentId });
            loadKits(); // Refresh
        } catch (error) {
            console.error('Failed to move kit', error);
            alert('Failed to reassign kit.');
        }
    };
    ...
    const handleDeleteKit = async (id) => {
        if (confirm('Are you sure you want to delete this kit?')) {
            await deleteKit(id);
            await deleteKitItemsByKitId(id);
            loadKits();
        }
    };
    ...
    const saveKitUpdates = async (kitId) => {
        try {
            ...
            await Promise.all(updates.map(id =>
                updateKitItem(parseInt(id), { serialNumber: itemsToUpdate[id].serialNumber }, user)
            ));
    ...
    // Initialize groups for ALL active deployments (filtered by context)
    // deployments from context is already filtered by selectedDeploymentIds? 
    // Usually deployments in context is "all deployments". selectedDeploymentIds is separate.
    // Let's check if we need to filter.
    // user rule: "make sure the app conext filter still works on these headers"
    // Assuming `deployments` from usage is the full list, and we need to filter if context has selection.

    // Wait, useDeployment usually returns { deployments, selectedDeploymentIds, ... }
    // If we want to show only selected, we filter.
    const { selectedDeploymentIds } = useDeployment();

    const visibleDeployments = selectedDeploymentIds && selectedDeploymentIds.length > 0
        ? deployments.filter(d => selectedDeploymentIds.includes(d.id))
        : deployments;

    visibleDeployments.forEach(d => groupedKits[d.id] = []);

    // Also handle kits for unlisted or "Unassigned" deployments
    const unassignedId = 'unassigned';
    groupedKits[unassignedId] = [];

    kits.forEach(k => {
        // If kit's deployment is visible, add to it.
        // If kit's deployment is NOT in visible list, do we show it?
        // Usually context filter hides other stuff. 
        // But if kit is in "unassigned", we might want to show that group if we want to drag FROM it?
        // For now let's stick to showing active deployments.

        if (groupedKits[k.deploymentId]) {
            groupedKits[k.deploymentId].push(k);
        } else if (!k.deploymentId && k.deploymentId !== 0) {
            // Handle unassigned
            groupedKits[unassignedId].push(k);
        } else {
            // Kit belongs to a deployment NOT in the current filter.
            // We can skip showing it, or show it. 
            // Requirement says "filter still works", so we skip.
        }
    });
    ...
    return (
        <div className="max-w-6xl mx-auto pb-20">
            ...
            <div className="space-y-6">
                {/* Render Visible Deployments + Unassigned */}
                {[...visibleDeployments, { id: 'unassigned', name: 'Unassigned Kits' }].map(deployment => {
                    const depId = deployment.id;
                    // Skip 'unassigned' block if empty and not in "admin" mode? 
                    // Or always show to allow dropping into Unassigned?
                    // User said "add headers for each deployment even if they don't have kits assigned"
                    // Doesn't explicitly say Unassigned, but it's good practice for DnD.

                    const deploymentKits = groupedKits[depId] || [];
                    const isExpanded = expandedDeployments[depId] !== false; // Default open?

                    // Visual feedback for drag over
                    const isDragOver = dragOverDeployment === depId;

                    return (
                        <div
                            key={depId}
                            className={`card overflow-hidden transition-all duration-200 ${isDragOver ? 'ring-2 ring-primary ring-offset-2 ring-offset-bg-primary' : ''}`}
                            onDragOver={(e) => handleDeploymentDragOver(e, depId)}
                            onDragLeave={handleDeploymentDragLeave}
                            onDrop={(e) => handleKitDrop(e, depId === 'unassigned' ? '' : depId)}
                        >
                            {/* Outer Container: Ship/Deployment */}
                            <div
                                className={`p-4 flex items-center justify-between cursor-pointer select-none transition-colors ${isDragOver ? 'bg-primary/20' : 'bg-bg-tertiary'}`}
                            ...
                            >
                                ...
                                {isDragOver && <span className="text-primary font-bold animate-pulse">Drop to Assign</span>}
                            </div>
                                ...
                                <div className="p-4 space-y-4">
                                    ...
                                    {deploymentKits.map(kit => {
                                        ...
                                        return (
                                            <div
                                                key={kit.id}
                                                className={`border border-border rounded-lg overflow-hidden bg-bg-secondary/20 transition-all ${canEdit ? 'cursor-grab active:cursor-grabbing' : ''}`}
                                                draggable={canEdit}
                                                onDragStart={(e) => handleKitDragStart(e, kit)}
                                            >
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        {canEdit && (
                                                            <>
                                                                <button
                                                                    onClick={(e) => { e.stopPropagation(); openAssignmentModal(kit); }}
                    ...
                    <div className="modal-backdrop">
                        <div className="modal max-w-md">
                            <div className="modal-header">
                                <h3 className="modal-title">Info: {kitToAssign?.name}</h3>
                            </div>
                            <div className="modal-body space-y-4">
                                <p className="text-sm text-muted">Select a new deployment for this kit.</p>
