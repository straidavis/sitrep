...
import { Calendar, Filter, Activity, Clock, Plane, Target, CheckCircle2 } from 'lucide-react';
...
const Reports = () => {
    ...
    const summaryStats = useMemo(() => {
        const metrics = calculateFlightMetrics(dateFilteredFlights);
        return {
            totalFlights: metrics.totalFlights,
            totalHours: metrics.totalHours, // already fixed to 1 decimal in util
            mrr: metrics.missionReliability, // Map missionReliability to mrr
            availability: metrics.availability,
            onTimeRating: metrics.onTimeRating,
            tois: metrics.totalTOIs
        };
    }, [dateFilteredFlights]);
    ...
    const comboData = useMemo(() => {
        ...
        dateFilteredFlights.forEach(flight => {
            ...
            if (viewMode === 'week') {
                ...
                key = format(weekStart, 'MMM dd'); // e.g., "Dec 01"
            ...
            if (!grouped[key]) {
                grouped[key] = {
                    period: key,
                    sortKey: sortKey, // Store ISO string for sorting
                    totalHours: 0,
                    totalFlights: 0,
                    delayedFlights: 0,
                    cancelledFlights: 0,
            ...
            if (['Completed', 'Complete'].includes(flight.status)) grouped[key].completedFlights = (grouped[key].completedFlights || 0) + 1;
            if (flight.status && flight.status.toLowerCase().includes('delay') && parseFloat(flight.hours || 0) > 0) grouped[key].delayedFlights += 1;
            if (flight.status === 'CNX') grouped[key].cancelledFlights += 1;
            if (flight.status === 'Aborted') grouped[key].abortedFlights = (grouped[key].abortedFlights || 0) + 1;
        });

        // Convert to sorted array using sortKey
        ...
        return sortedData.map(item => {
            ...
            // "Completed" for the purpose of On-Time Rate includes both On-Time and Delayed flights (assuming Delay means "Flown but Late")
            // If item.completedFlights only counts "Complete" status (On-Time), then adding delayed creates the total "Flown" set.
            const flownFlights = (item.completedFlights || 0) + (item.delayedFlights || 0);

            const onTimeRating = flownFlights > 0
                ? ((flownFlights - item.delayedFlights) / flownFlights) * 100
                : 100;
    ...
    return (
        <div className="pb-20">
            ...
            {/* Summary Stats */}
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-lg mb-8">
                ...
                <div className="stat-card">
                    ...
                    <div className="stat-value">{summaryStats.availability || '100.0'}%</div>
                </div>
                <div className="stat-card">
                    <div className="stat-header">
                        <div className="stat-icon">
                    ...
                    <div className="stat-value">{summaryStats.onTimeRating || '100.0'}%</div>
                </div>
                <div className="stat-card">
                    <div className="stat-header">
                        <div className="stat-icon">
                ...
                <div className="card col-span-1 lg:col-span-2">
                    <div className="card-header flex items-center justify-between">
                        ...
                        <div className="flex gap-4 text-sm text-text-muted">
                            ...
                            <span className="flex items-center gap-2">
                                <span className="w-3 h-3 bg-accent-secondary rounded-full"></span> On-Time
                            </span>
                        </div>
                    </div>
                    <div className="card-body" style={{ height: '400px' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={comboData}>
                                ...
                                <Line yAxisId="right" type="monotone" dataKey="onTimeRating" name="On-Time %" stroke="var(--color-accent-secondary)" strokeWidth={3} dot={{ r: 4 }} />
                            </ComposedChart>
                        </ResponsiveContainer>
                    </div>
                </div>
