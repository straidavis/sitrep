...
import { getUserByEmail, addUser } from '../db/users'; // Updated import
...
const DeploymentForm = ({ deployment, onSave, onCancel }) => {
    ...
    useEffect(() => {
        if (deployment) {
            setFormData({
                ...
                equipment: (Array.isArray(deployment.equipment) ? deployment.equipment : []).join(', '),
                userEmails: (Array.isArray(deployment.userEmails) ? deployment.userEmails : []).join(', '),
    ...
    const handleAddEmail = async () => {
        ...
        if (!currentList.includes(emailToAdd)) {
            ...
            // 2. SYNC LOGIC: Check/Add to Admin Portal
            try {
