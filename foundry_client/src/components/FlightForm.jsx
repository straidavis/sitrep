...
const FlightForm = ({ flight, onSave, onCancel }) => {
    ...
    const statuses = ['Complete', 'CNX', 'Delay', 'Alert - No Launch', 'Aborted'];
    ...
    useEffect(() => {
        if (formData.deploymentId) {
            const dep = deployments.find(d => String(d.id).trim() === String(formData.deploymentId).trim());
    ...
    useEffect(() => {
        const generateMissionNumber = async () => {
            if (formData.date && !flight) {
                ...
                // const yy = year.slice(-2); // Unused

                // Format: M-YYYYMMDD
                let prefix = `M-${year}${month}${day}`;

                // Note: Removed Aircraft suffix logic to match standard format M-YYYYMMDD-XX
    ...
    const handleChange = (e) => {
        ...
        setFormData(prev => {
            ...
            // Clear fields only if Alert - No Launch or CNX (Aborted allows inputs)
    ...
    const validate = () => {
        ...
        // 1. Hide Fields: CNX and Alert - No Launch hide most fields. Aborted SHOWS them.
        // 2. Relax Requirements: CNX, Alert - No Launch, AND Aborted have relaxed requirements (only need Reason).

        const hideFields = ['CNX', 'Alert - No Launch'].includes(formData.status);
        const relaxRequirements = ['CNX', 'Alert - No Launch', 'Aborted'].includes(formData.status);
        ...
        // Require Reason if CNX, Delay, or Aborted
        if (['CNX', 'Delay', 'Aborted'].includes(formData.status)) {
    ...
    const handleSubmit = (e) => {
        ...
        const flightData = {
            ...
            deploymentId: formData.deploymentId ? String(formData.deploymentId).trim() : null,
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-3" style={{ gap: 'var(--spacing-lg)' }}>
                ...
                {/* Aircraft Number */}
                <div className="form-group">
                    <label className="form-label">Aircraft # {(!['CNX', 'Alert - No Launch', 'Aborted'].includes(formData.status)) && '*'}</label>
                    <select
                        name="aircraftNumber"
                        className="select"
                    ...
                    >
                        ...
                        {aircraftList.map((item, idx) => (
                            <option key={item.serial || idx} value={item.serial}>{item.label}</option>
                        ))}
                    </select>
                ...
                <div className="form-group">
                    ...
                    >
                        ...
                        {deployments.map((deployment, idx) => (
                            <option key={deployment.id || `dep-${idx}`} value={deployment.id}>
            ...
            <div style={{
                ...
                <div className="grid grid-cols-3" style={{ gap: 'var(--spacing-md)' }}>
                    <div className="form-group" style={{ marginBottom: 0 }}>
                        <label className="form-label">Payload 1 {(!['CNX', 'Alert - No Launch', 'Aborted'].includes(formData.status)) && '*'}</label>
                        <select
                            name="payload1"
                            className="select"
            ...
            <div className="grid grid-cols-2" style={{ gap: 'var(--spacing-lg)' }}>
                <div className="form-group">
                    <label className="form-label">REASON for Cancel, Abort or Delay {(['CNX', 'Delay', 'Aborted'].includes(formData.status)) && '*'}</label>
                    <select
                        name="reasonForDelay"
                        className="select"
            ...
            <div className="grid grid-cols-3" style={{ gap: 'var(--spacing-lg)' }}>
                {/* TOIs */}
                <div className="form-group">
                    <label className="form-label">TOIs {(!['CNX', 'Alert - No Launch', 'Aborted'].includes(formData.status)) && '*'}</label>
                    <input
                        type="number"
                        name="tois"
                        className="input"
